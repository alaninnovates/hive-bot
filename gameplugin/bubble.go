	...
	for i := range user.Bubbles {
		...
		for j := range user.Bubbles[i] {
			...
			buttons = append(buttons, discord.ButtonComponent{
				Style:    style,
				CustomID: "/game/pop-bubbles/bubble/" + userId.String() + "/" + strconv.Itoa(i) + "/" + strconv.Itoa(j),
				Label:    " ",
...
func BubbleCommand(b *common.Bot, gameService *State) handler.CommandHandler {
...
func BubbleButton(gameService *State) handler.ComponentHandler {
	return func(event *handler.ComponentEvent) error {
		...
		row, _ := strconv.Atoi(event.Vars["i"])
		col, _ := strconv.Atoi(event.Vars["j"])
